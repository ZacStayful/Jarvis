...
- **Header Auth account 4** — Monday API Key (`Authorization` header)
- **Header Auth account 6** — GitHub PAT (Bearer token for `ZacStayful/stayful-presentations`)
...
> - Strip these fields from the GET response before PUT-ing: `availableInMCP`, `binaryMode`
