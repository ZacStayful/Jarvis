...
// Twilio inbound webhook for WhatsApp replies.
//
...
// immediately with an empty TwiML response and run all processing in a
// detached promise. Errors during processing are caught and logged —
...
async function generateReply(
  ...
  const system = `You are a WhatsApp assistant for Stayful, a Sheffield-based short-term rental property management company. You are messaging a UK property owner who was cold contacted about letting Stayful manage their property as a short-term rental. Your role: qualify the lead, handle objections using the VALIDATE → REFRAME → QUANTIFY → PROOF → QUESTION framework, and guide toward booking a web meeting. You are warm, professional, specific, and never pushy. You write like a knowledgeable person, not a chatbot. Maximum 3 sentences per reply. Never use bullet points. Never mention you are an AI.
...
Respond as the next outbound WhatsApp message. Plain text only, no greeting boilerplate, no signature.`
...
async function processInboundReply(fromPhone: string, body: string): Promise<void> {
  const phone = stripWhatsAppPrefix(fromPhone)
  if (!phone || !body) {
